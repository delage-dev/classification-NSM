...
mesh_list_raw = random.sample(val_paths, min(1, len(val_paths)))
mesh_list = []
...
for i, vert_fname in enumerate(mesh_list):    
    ...
    # Extract KNN predictions for printing
    ...
    with open(results_fpath, "w") as f:
        ...
        for clf_name, clf_model in trained_models.items():
            ...
            if probabilities[clf_name] is not None:
                # probabilities is a list of arrays (one for each target)
    ...
    # Get species parsing
    parsed_truth = parse_taxonomy_from_filename(os.path.basename(vert_fname))
    mesh_species = parsed_truth.get('species') if parsed_truth else extract_species_prefix(os.path.basename(vert_fname))
...
df.to_csv("summary_matches_v2_95pct_cos.csv", index=False)
print("Summary saved to summary_matches_v2_95pct_cos.csv")